...
int
evaluate(struct evaluate_context *ec)
{
    ...
    while (program_counter < ec->instructions_count) {
        ...
        case LOAD_CONST:
        {
            stack_push(interpreter, instruction.arg);
            break;
        }
        ...
        case ADD_UINT:
        {
            union fds_filter_value b = stack_pop(interpreter);
            union fds_filter_value a = stack_pop(interpreter);
            a.uint_ += b.uint_;
            stack_push(interpreter, a);
            break;
        }
